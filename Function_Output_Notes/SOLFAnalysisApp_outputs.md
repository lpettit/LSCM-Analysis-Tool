...
| `WholeImageSlicesCheckBox` | Selects the whole-image height-slice Module 3 tab. | This shows whole-surface area, perimeter, and cumulative surface area versus height. |
...
| `DoneButton` | Accepts the current mound-detection result. | This saves `bestParams_<imageName>.mat` to the selected output folder. |
...
The Module 3 GUI exports additional tab review products under `mound_surface_gui_exports/` in the selected output folder only when the user clicks `Save Current Tab` or `Save All`.

function results = analyzeMounds(imagePath, bestParams, fillDeepPits, fillThreshold, ...
                                  dilateRadius, minObjectArea, xy_um_per_px, ...
                                  total_height_um, outputDir)
% =========================================================================
%  analyzeMounds  —  Module 1: Spacing, density, and Delaunay analysis
%
...
if is_vk4
    ...
    if ~exist('readVK4', 'file')
        error(['analyzeMounds: readVK4.m not found on MATLAB path.\n' ...
    ...
    if isempty(xy_um_per_px) || isempty(total_height_um)
        error(['analyzeMounds: xy_um_per_px and total_height_um are required ' ...
...
if n_mounds < 4
    error('analyzeMounds: fewer than 4 mounds detected — check parameters.');
end
...
fprintf('analyzeMounds complete.\n\n');
